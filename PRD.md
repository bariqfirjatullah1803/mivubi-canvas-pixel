...
>
> **v1.2**: §1–§24 tetap merekam produk sumber dan tidak diubah isinya. Yang baru adalah **§25**, yang mencatat keputusan rebuild (R1–R21) yang sudah diterapkan di prototipe UI, plus penanda `[R#]` di klausa yang keputusannya menggantikan perilaku sumber. DESIGN.md v1.2 memuat spesifikasi visualnya.
...
| R21 | **Referensi dipajang di Canvas World sebagai karya MIVUBI.** Tiap referensi punya satu karya publik milik akun sistem `MIVUBI` (tidak bisa dipakai masuk). Karya itu dibuat atau diperbarui setiap referensi disimpan, ikut terhapus saat referensinya dihapus, dan dibuat otomatis untuk referensi lama saat server menyala | §8.3 (F2), §13.6, Lampiran A | Referensi adalah karya Admin; memajangnya membuat World dan papan landing tidak kosong di awal pameran. Memakai tabel `projects` membuat penempatan, dialog detail, halaman `/art`, dan moderasi berlaku tanpa kode baru. Takedown oleh Admin tetap berlaku walau referensinya diubah | `src/lib/server/templates.ts`, `src/hooks.server.ts`, `src/routes/api/templates/*`, `db/migrations/0004_template_artworks.sql` |
